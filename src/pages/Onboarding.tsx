...
export default function Onboarding() {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return <ProfileAutofillStep profileData={onboardingData.profile} onNext={data => {
          if (data) {
            setOnboardingData({
              ...onboardingData,
              profile: {
      ...
      case 3:
        return <ConsentsStep initialConsents={onboardingData.consents} onNext={consents => {
          setOnboardingData({
            ...onboardingData,
            consents
          });
      ...
      case 4:
        return <SubscriptionStep onNext={planId => {
          setOnboardingData({
            ...onboardingData,
            subscription: planId
          });
      ...
      case 6:
        return <NutritionStep onNext={nutrition => {
          setOnboardingData({
            ...onboardingData,
            nutrition
          });
      ...
      case 8:
        return <PreferencesStep onNext={prefs => {
          setOnboardingData({
            ...onboardingData,
            preferences: prefs
          });
  ...
  return <div className="min-h-screen bg-background p-6">
      <div className="max-w-3xl mx-auto space-y-8 py-[20px]">
        {/* Back Button */}
        {currentStep > 1 && <button onClick={prevStep} className="fixed top-6 left-6 w-10 h-10 rounded-full bg-card/80 backdrop-blur-xl border border-primary/10 shadow-card hover:shadow-glow hover:bg-card transition-all duration-300 flex items-center justify-center group z-50" aria-label="Go back" style={{
        marginTop: '0px'
      }}>
        ...
        {/* Close Button */}
        {currentStep > 0 && <button onClick={handleSkip} className="fixed top-6 right-6 w-10 h-10 rounded-full bg-card/80 backdrop-blur-xl border border-primary/10 shadow-card hover:shadow-glow hover:bg-card transition-all duration-300 flex items-center justify-center group z-50" aria-label="Skip onboarding" style={{
        marginTop: '0px'
      }}>
        ...
        {/* Progress Indicator */}
        {currentStep > 0 && <div className="space-y-3">
            ...
            <div className="h-2 rounded-full bg-muted overflow-hidden border border-primary/10">
              <div className="h-full rounded-full bg-gradient-to-r from-[#12AFCB] to-[#12AFCB]/90 transition-all duration-500 ease-out" style={{
