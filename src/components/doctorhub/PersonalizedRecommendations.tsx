...
export const PersonalizedRecommendations = ({
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      const {
        data: recData,
        error: recError
      } = await supabase.functions.invoke('suggest-test-sets');
      if (recError) {
      ...
      if (recs.length > 0) {
        ...
        const {
          data: setsData,
          error: setsError
        } = await supabase.from('test_sets').select('*').in('id', testSetIds);
        ...
        const setsWithCounts = await Promise.all((setsData || []).map(async set => {
          const {
            count
          } = await supabase.from('test_set_items').select('*', {
            count: 'exact',
            head: true
  ...
  return <div className="space-y-6 mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        return <TestSetCard key={rec.test_set_id} testSet={testSet} reasoning={rec.reasoning} priority={rec.priority} onOrderSuccess={onOrderSuccess} />;
      })}
      </div>
    </div>;
};