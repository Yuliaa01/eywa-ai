...
export function EducationLibrary({ onVideoSelect, className }: EducationLibraryProps) {
  ...
  const filteredVideos = videos.filter((video) => {
    ...
    if (searchQuery) {
      ...
      const matchesSearch = 
        ...
        video.health_domains.some((domain) => domain.toLowerCase().includes(query));
    ...
    if (selectedCategory) {
      const matchesCategory = video.health_domains.some(
        (domain) => domain.toLowerCase() === selectedCategory.replace("-", "_")
      );
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      {/* Search and filters */}
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-md p-4 space-y-4 border-b border-border/30">
        ...
        <div className="flex gap-2">
          <Button
            variant={contentFilter === null ? "default" : "outline"}
            size="sm"
          ...
          <Button
            variant={contentFilter === "education" ? "default" : "outline"}
            size="sm"
            onClick={() => setContentFilter("education")}
          >
            Education
          </Button>
          <Button
            variant={contentFilter === "tutorial" ? "default" : "outline"}
            size="sm"
            onClick={() => setContentFilter("tutorial")}
          >
            Tutorials
          </Button>
          <Button
            variant={contentFilter === "reel" ? "default" : "outline"}
            size="sm"
      ...
      <div className="flex-1 overflow-auto p-4">
        {loading ? (
          ...
          <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {educationVideos.map((video) => (
                    <VideoCard
                      ...
                      creatorAvatarUrl={video.creator_avatar_url || undefined}
                      viewsCount={video.views_count}
                      contentType={video.content_type}
                      healthDomains={video.health_domains}
            ...
            {/* Reels section */}
            {reelVideos.length > 0 && contentFilter !== "education" && contentFilter !== "tutorial" && (
              <section>
