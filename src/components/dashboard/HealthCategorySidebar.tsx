...
export function HealthCategorySidebar({
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        onClick={() => onCategoryClick("chat")}
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all border shadow-sm ${
          activeCategory === "chat"
            ? "bg-[#12AFCB] text-white border-[#12AFCB]"
            : "bg-white/80 backdrop-blur-xl text-[#5A6B7F] hover:bg-gray-50 border-gray-100"
        }`}
      >
        <MessageSquare className="w-5 h-5 flex-shrink-0" />
        <span className="text-sm font-semibold">AI Health Coach</span>
      </button>
    </div>
  );
}
